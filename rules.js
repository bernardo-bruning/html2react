'use-strict';
const {assoc, omit, filter, join, defaultTo, isEmpty, prop} = require('ramda');
...
const rules = [
	...
	{
		is: e => e.name === 'div' && e.attributes && e.attributes.className && e.attributes.className.includes('row'),
		build: e => {
			const attributes = join(' ', filter(x => !x.includes('row'), e.attributes.className.split(' ')));
			return {
				type: 'element',
				name: 'Row',
				elements: e.elements || [],
				attributes: attributes.length > 0 ?
					assoc('className', join(attributes), omit(['className'], e.attributes)) :
					omit(['className'], e.attributes)
			};
		}
	},
	{
		is: e => e.name === 'div' && e.attributes && e.attributes.className && e.attributes.className.includes('col-'),
		build: e => (
			{
				type: 'element',
				name: 'Col',
